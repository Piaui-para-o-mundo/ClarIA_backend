...
def _replace_dispatch_placeholders(
...
async def pdf_preview(
    ...
    processo = await ProcessoService.get_processo(
        db=db, processo_id=str(processo_id)
    )
    if not processo:
    ...
    try:
        ...
        return Response(
            ...
            headers={
                'Content-Disposition': 'inline; filename=preview_despacho.pdf'
            },
        )
...
async def render_preview(request: Request, payload: DispatchPayload):
    ...
    return templates.TemplateResponse(
        request=request, name='dispatch.html', context=context
    )
...
async def send_despacho_to_processo(
    ...
    try:
        ...
        print(
            f'[DISPATCH SEND] Erro ao renderizar template:\n{traceback.format_exc()}',
            flush=True,
        )
        raise HTTPException(
    ...
    try:
        ...
        print(
            f'[DISPATCH SEND] Erro ao gerar/salvar despacho:\n{traceback.format_exc()}',
            flush=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    await ProcessoService.save_despacho_automatico(
        db=db, processo_id=str(processo_id), despacho=payload.corpo_despacho
    )
    await db.commit()
    ...
    print(
        f'[DISPATCH SEND] Documento salvo: id={doc_id}, caminho={documento.caminho_arquivo}',
        flush=True,
    )
