...
class AnaliseService:
    ...
    @staticmethod
    async def obter_processo_lock(
        db: AsyncSession, processo_id: UUID
    ) -> Processo | None:
        stmt = (
            select(Processo)
    ...
    async def executar_analise_automatica(
        ...
        processo = await AnaliseService.obter_processo_lock(
        ...
        if not processo.documentos:
            ...
            AnaliseService._append_log(
                processo, 'Análise aguardando documentos para submissão.'
            )
            await db.flush()
            return processo
        ...
        AnaliseService._append_log(
            processo, 'Análise automática iniciada (etapa 1).'
        )
        await db.flush()
        ...
        try:
            ...
            if not docs_para_rag:
                raise ValueError(
                    'Não foi possível ler o conteúdo dos PDFs no disco.'
                )

            print(
                f'[ANALISE BACKGROUND] Iniciando Etapa 1: Conformidade para {processo_id}',
                flush=True,
            )
            AnaliseService._append_log(
                processo, 'Enviando PDFs para /ia/conformidade...'
            )
            ...
            processo.checklist_ia = json.dumps(
                conformidade, ensure_ascii=False
            )
            ...
            AnaliseService._append_log(
                processo,
                'Checklist concluído. Resumo e despacho pendentes de solicitação manual.',
            )
            ...
            AnaliseService._append_log(
                processo,
                'Análise automática IA (etapa 1) concluída com sucesso.',
            )
            return processo
            ...
            AnaliseService._append_log(
                processo, f'Falha na comunicação com RAG: {exc}'
            )
            await db.flush()
            return processo
    ...
    async def gerar_resumo(
        ...
        # Etapa 2: Resumo
        print(
            f'[ANALISE MANUAL] Iniciando Etapa 2: Resumo para {processo_id}',
            flush=True,
        )
    ...
    async def gerar_despacho(
        ...
        # Etapa 3: Despacho
        print(
            f'[ANALISE MANUAL] Iniciando Etapa 3: Despacho para {processo_id}',
            flush=True,
        )
        ...
        try:
            ...
            processo.despacho_automatico = json.dumps(
                despacho_response, ensure_ascii=False
            )
            AnaliseService._append_log(
                processo, 'Despacho manual concluído com sucesso.'
            )
        except httpx.HTTPError as exc:
            processo.despacho_automatico = (
                'Erro de comunicação com a API do RAG.'
            )
            AnaliseService._append_log(
                processo, f'Falha no RAG ao gerar despacho: {exc}'
            )
    ...
    async def disparar_analise_em_background(processo_id: UUID) -> None:
        ...
        async with session_factory() as db:
            try:
                ...
                await db.commit()
                print(
                    f'[ANALISE BACKGROUND] Processo {processo_id} finalizado.',
                    flush=True,
                )
            except Exception as exc:
                await db.rollback()
