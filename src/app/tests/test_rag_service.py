...
def _response(status_code: int, payload: dict | None = None) -> httpx.Response:
    ...
    return httpx.Response(
        status_code=status_code, json=payload or {}, request=request
    )
...
async def test_gerar_resumo_retorna_fallback_legado_quando_api_responder_422():
    ...
    resultado = await client.gerar_resumo(
        ...
        textos_extraidos=[
            {'nome': 'scan001.pdf', 'texto': 'Conteudo extraido'},
        ],
    )
...
async def test_gerar_resumo_nao_retry_sem_texto_legado():
    client = RagClient(base_url='http://rag.local')
    client.client.post = AsyncMock(
        return_value=_response(422, {'detail': 'Validation error'})
    )
...
@pytest.mark.asyncio
async def test_sugerir_despacho_retorna_fallback_legado_quando_api_responder_422():
    client = RagClient(base_url='http://rag.local')
    client.client.post = AsyncMock(
        side_effect=[
            _response(422, {'detail': 'Validation error'}),
            _response(200, {'despacho': 'Despacho legado'}),
        ]
    )

    resultado = await client.sugerir_despacho(
        checklist_result={
            'aprovado': False,
    ...
    assert resultado == {'despacho': 'Despacho legado'}
    assert client.client.post.await_count == 2
    assert client.client.post.await_args_list[0].kwargs['json'] == {
        'checklist_result': {
            'aprovado': False,
